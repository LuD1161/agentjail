...
**Filter support:** `store.Filter` supports `DecisionID` (exact row),
`SessionID` (substring),
...
`network.db` handle; `session_log_snapshot` serves a byte-bounded exact-session
timeline from the daemon's existing event store; and `session_action_detail`
serves one exact session/decision pair after a row is opened. These projections omit full
paths, request headers/bodies/query strings, and general tool input. An opened
Bash action detail may include only the bounded command parsed from the
store-redacted JSON; commands never ride in the repeating timeline and raw
input is never persisted or projected. See
...
ADR 0121-current-model-pricing and ADR 0123-supplemental-model-pricing).
...
the same transcript remain readable (ADR 0122-transcript-record-recovery).

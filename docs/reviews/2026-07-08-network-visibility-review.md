...
> | Mechanism A (host-veth + privileged daemon) | **Deleted** per ADR 0079; **AGE-103/AGE-140 obviated**. `c6687a3` |
>
...
Session-scoped isolation primitive is correct on Linux (`CLONE_NEWUSER|NEWNET|NEWNS`); CA injection is host-untouched by construction; host-side fail-open on Linux is clean; upstream TLS identity is verified; the protocol parsers are panic-hardened with a real chaos suite; ADR 0079 is a model deprecation record. The bones are right — the integration and the fail-*closed* posture are what's missing.

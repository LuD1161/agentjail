...
- **Linux:** Lima/QEMU on the home server (`LIMA_HOME=/DATA/lima`), Ubuntu LTS
...
- **Shell scripts, not a framework:** `testbed.sh` verbs
  ...
  the shell version demonstrably hurts (see Linear AGE-146 for the staged
...
- Release confidence: the exact `curl | sh` user flow is exercised on a clean
  box before tagging (manual at first, automated in AGE-146 stages 3–4).

...
follow-up). Phase 3 (auto-recovery) is a separate follow-up — see
...
the daemon now re-arms on startup — see the U2 fix). This matches follow-up's
...
1. **Silent false sense of security.** A user who installed agentjail to be
   ...
   `brew upgrade` that doesn't reload launchd — follow-up, or a deliberate DoS —
   P9/follow-up). The OS shield still enforces files+network, but *all*
...
the above matters. Scoped as a follow-up phase (ties into follow-up) so it doesn't
...
- Updates the "never block the agent" stance from **follow-up** — that remains the
  `allow` default, but is no longer the only option.
- Complements **U2** (sentinel re-arm), **P9/follow-up** (DoS hardening), and
  **follow-up** (daemon restart on upgrade).

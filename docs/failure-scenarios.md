...
| A3 | **Installed unit predates ADR 0070** (`Restart=on-failure`) and is never rewritten | Upgrade rewrites the unit, or doctor flags the stale one | UNVERIFIED — needs a live test. `install.go:1946` writes the unit on install; whether `agentjail update` re-runs `installSystemdUnit` on an existing install, and whether a stale `Restart=on-failure` unit is detected, is not determinable without running the upgrade path. **This is a candidate root cause for AGE-212's "launchd never restarted it"**, which `KeepAlive=<true/>` should otherwise have prevented. | No check reads the on-disk unit/plist contents | none | **P0** |
...
| F4 | Policy says `ask`, agent is Codex | Honest failure | Collapses to **deny** — Codex PreToolUse cannot prompt (`main.go:626-629`). Documented in ADR 0050's caveat. | Yes, loud | none | P2 |
...
| H3 | Agent unlinks `~/.agentjail/daemon.sock` | Blocked by the sandbox | UNVERIFIED — needs a live test. Landlock grants `~/.agentjail` read-only with a single-file write grant on `daemon.sock` (`ARCHITECTURE.md`); unlink needs write on the *directory*, which is not granted. Not verified live, and macOS parity is G5. | No | none | P1 |

...
//   the conn-handle API (wg_netstack_send / wg_netstack_recv).
//
...
class TransparentProxyProvider: NETransparentProxyProvider {
    ...
    override func startProxy(options: [String: Any]?,
        ...
        if rc != 0 {
            ...
            completionHandler(NSError(domain: "agentjail", code: 2,
                userInfo: [NSLocalizedDescriptionKey: "wg-netstack: \(msg)"]))
            return
        }

        // IPC listener -- synchronous handshake from agentjail CLI
        ...
        //   - Tunnel flows (bridgeTCP/bridgeUDP for agentjail children)
    ...
    private func applyNetworkSettings(completionHandler: @escaping (Error?) -> Void) {
        ...
        // Flow. Per-process agentjail children's UDP/53 is tunneled
    ...
    private func bridgeTCP(_ flow: NEAppProxyTCPFlow) {
        ...
        flow.open(withLocalEndpoint: nil) { err in
            ...
            let cid = ip.withCString { hostC in
                errBuf.withUnsafeMutableBufferPointer { ebuf in
                    wg_netstack_tcp_connect(UnsafeMutablePointer(mutating: hostC), port, 15000,
                                            ebuf.baseAddress, Int32(ebuf.count))
                }
            }
            if cid < 0 {
    ...
    /// Resolve hostname -> IPv4. Routes through the tunnel
    /// (1.1.1.1:53 via netstack). Already-IP literals short-circuit
    /// on the Go side.
...
// Session registry -- populated synchronously by `agentjail run`
// (Go) over the unix socket at sessionSockPath before the wrapped
// child exec's. Transport-agnostic so the Go CLI can speak it
...
// agentjail PID just means the ext might tunnel that PID's flows,
// a local authz concern only the host's user controls anyway.
let sessionSockPath = "/tmp/agentjail.sock"
...
private func sessionRegister(_ pid: pid_t) {
    ...
    // after `agentjail run` a cold-cache storm: every Chrome
...
// The path-based check previously used was the actual cause of the
// Chrome-hangs-during-agentjail-run symptom: proc_pidpath is
// ~50-200us per level, and Chrome spawns many helper processes whose
// flows all hit handleNewFlow. Walking 5-10 levels x hundreds of
// concurrent flows x the path syscall = handleNewFlow returning
// slowly enough to mis-route UDP. proc_pidinfo is ~5us and Set
...
// flip a non-agentjail flow into "tunneled" or vice versa.
//
...
// BypassUDP -- non-tunnel UDP flows from non-agentjail processes.

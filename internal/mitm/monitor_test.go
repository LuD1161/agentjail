...
func TestMonitorForwardsAndRecordsHTTPVerdicts(t *testing.T) {
	for _, proto := range []string{"http/1.1", "h2"} {
		for _, verdict := range []string{"deny", "ask"} {
			t.Run(proto+"/"+verdict, func(t *testing.T) {
				...
				var err error
				h.Matcher, err = netpolicy.NewMatcherForMode(agentconfig.EnforcementMonitor, dir)
				if err != nil {
					t.Fatal(err)
				}
				...
				if resp.StatusCode != 200 || string(body) != "upstream response" || reached.Load() != 1 {
					t.Fatalf("status=%d body=%q upstream calls=%d", resp.StatusCode, body, reached.Load())
				}
				select {
				case row := <-logs:
					if row.PolicyAction != "allow" || row.WouldAction != verdict || row.PolicyTemplate != "monitor-test" || row.StatusCode != 200 {

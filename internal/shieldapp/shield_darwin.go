...
func generateSBProfileWithCapabilities(cfg *config.PolicyConfig, home string, allowedIPs []string, withNetproxy bool, capabilities darwinProfileCapabilities) string {
	...
	appendDarwinNetworkProfile(&sb, home, withNetproxy, capabilities)

	return sb.String()
}

// appendDarwinFilesystemProfile renders the shared filesystem contract for
// every Darwin shield profile; network policy is rendered separately.
func appendDarwinFilesystemProfile(sb *strings.Builder, cfg *config.PolicyConfig, home string, capabilities darwinProfileCapabilities) {
...
func appendDarwinNetworkProfile(sb *strings.Builder, home string, withNetproxy bool, capabilities darwinProfileCapabilities) {

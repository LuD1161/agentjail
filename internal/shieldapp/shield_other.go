...
func runShield(cfg *config.PolicyConfig, agentPath string, agentArgs []string, profilePrint bool, noNetproxy bool, tunnelMode bool, requireTunnel bool, mitmMode bool, ipv6Mode bool, sshAuthSock sandbox.SSHAuthSock, credentialTools credentialSelections, policyPath string, _ time.Time, emitter audit.Emitter) {
	if profilePrint {
	...
	if requireTunnel {
		fmt.Fprintln(os.Stderr, "agentjail-shield: required transparent tunnel is unsupported on this platform; refusing to launch")
		os.Exit(1)
	}

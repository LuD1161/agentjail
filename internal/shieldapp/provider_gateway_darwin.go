...
func providerGatewayWanted(cfg *config.PolicyConfig, agentPath string) (capturegw.Provider, bool) {
	prov, ok := capturegw.Lookup(filepath.Base(agentPath))
	if !ok || !prov.Caps.Verified || !prov.Caps.BaseURLEnv {
		return capturegw.Provider{}, false
	}
...
// loopback capture gateway (internal/gateway) so its /v1/messages traffic is
...
func startProviderGateway(ctx context.Context, cfg *config.PolicyConfig, agentPath string, rec capturegw.Recorder, bodies *mitm.BodyStore, sessionID string, logger *slog.Logger, emitter audit.Emitter) (env map[string]string, closeFn func() error, started bool, err error) {
	...
	target, terr := capturegw.ResolveForwardTarget(inherited, prov, allowlist)
	if terr != nil {
	...
	gw := capturegw.New(target, rec, capturegw.Options{
		SessionID: sessionID,
		OwnerPID:  os.Getpid(),
		Logger:    logger,
		Bodies:    bodies,

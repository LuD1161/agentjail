...
// nativeAudit pins the BPF arch guard to AUDIT_ARCH_AARCH64.
const nativeAudit uint32 = uint32(unix.AUDIT_ARCH_AARCH64)

// nativeSyscalls — aarch64 (asm-generic UAPI). Several legacy syscalls
// are deliberately absent on this arch and are NOT keys here:
//   open, stat, lstat, access, dup2, fork, pipe, poll, epoll_wait,
//   eventfd, arch_prctl, readlink (only readlinkat).
// The resolver in profile_linux.go skips those names silently
// (KnownNames check); they are uncallable on this kernel arch by
// definition.
var nativeSyscalls = map[string]uint32{
	// File I/O (no open/stat/lstat/access/dup2/readlink)
	...
	// Process lifecycle (no fork on arm64; clone covers it)
	...
	// I/O multiplexing (no poll/epoll_wait/eventfd)

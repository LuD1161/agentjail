...
func generateSBProfileWithIPs(cfg *config.PolicyConfig, home string, allowedIPs []string, withNetproxy bool) string {
	...
	// follow-up: daemon-ctl.sock is the daemon's grant control socket,
	...
	// destination (verified with sandbox-exec; a comparable macOS Seatbelt sandbox uses the same

...
### Codex authentication (current gate)
...
The gate requires a readable file-based Codex auth cache. Override its location
with `CODEX_AUTH_FILE=/path/to/auth.json`. If Codex uses the OS credential store,
configure file storage and log in before running the gate; the official Codex
authentication guide documents `cli_auth_credentials_store = "file"` and the
headless auth-cache copy workflow.

The default pins Codex CLI 0.147.0; override with
`AGENTJAIL_TESTBED_CODEX_VERSION`. Authentication is copied immediately before
the scenario rather than during provisioning, and both the guest scenario and
host runner remove it afterward, including on an interrupted host run. It does
not copy host Codex config, plugins, MCP definitions, or sessions. Do not record
or publish the guest filesystem while credentials are present.
...
provisions the current worktree through the real installer, and runs the
selected agent's live scenario. It **exits non-zero on insufficient host
resources, missing authentication, scenario failure, or install failure** and
deletes the gate VM afterward by default. Codex is the default. Run it on the
Linux host for the Linux build and on the Mac for the macOS build.

# agentjail testbeds — clean-VM sandboxes (AGE-146)
...
|---|---|---|---|
| **Linux** | **home server** (this box) | Lima/QEMU, `LIMA_HOME=/DATA/lima` | ✅ **DONE — set up and validated end-to-end on the home server. Do not redo.** |
| **macOS** | Aseem's Apple-Silicon Mac | Tart | ⬜ TODO — **this is the only part the Mac-side agent needs to do** (see "Mac side" below) |
...
## Linux side (home server) — DONE, for reference only

Installed on the server: QEMU (apt `qemu-system-x86 qemu-utils`), Lima v2.1.4
(static tarball → `/usr/local`), user in `kvm` group, `LIMA_HOME=/DATA/lima`
...
> home server. **Only do the macOS/Tart work below.** The shared scripts
...
On the laptop the same bare repo is named `homelab` (not `local-bare`):

```sh
cd ~/Repos/AgentJail-Repos/agentjail   # the local-dev working copy
git fetch homelab
git checkout security/2026-07-07-review-fixes
git pull homelab security/2026-07-07-review-fixes
```
...
- Stage 3 (scripted scenarios in `scenarios/`) and Stage 4 (nightly timers +
  Planka report) are tracked in Linear AGE-146 — see the "Lean rollout plan"
  comment there.

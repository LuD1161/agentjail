...
`no_hook_self_disable.rego` was removed. Hook config protection is now handled
by the core `file_policy/hook_config` rule in `file_policy.rego` (ask on
Write/Edit to `~/.claude/settings*.json` only; not locked, and does not cover
`~/.codex/` or `~/.cursor/`).
